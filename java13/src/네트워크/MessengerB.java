...
//			list.append(input.getText() + '\n');
			try {
				...
				DatagramPacket packet = new DatagramPacket(input.getText().getBytes("UTF-8"), input.getText().getBytes("UTF-8").length, ip, 7777);
//				DatagramPacket packet = new DatagramPacket(input.getText().getBytes(), input.getText().length(), ip, 7777);
				// 패킷 보내기
				socket.send(packet);
	...
	public void process() {
		while (true) {
			try {
				...
				System.out.println("MessengerB 가 받은 데이터 : " + new String(data));
//				System.out.println("MessengerB 가 받은 크기 : " + new String(data).length());
